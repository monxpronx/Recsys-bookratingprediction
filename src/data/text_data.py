...
def process_text_data(ratings, users, books, tokenizer, model, vector_create=False):
    ...
    # 라벨 생성
    users_['age_label'] = users_['age'].apply(label_age)
    #####################################################################
...
def text_data_load(args):
    ...
    ###################### 수정 ######################
    # 원래코드: book_features = []
    book_features = ['category', 'publisher']

    # 원래코드: sparse_cols = ['user_id', 'isbn'] + list(set(user_features + book_features) - {'user_id', 'isbn'})
    sparse_cols = ['user_id', 'isbn', 'age_label'] + book_features
    #################################################    
    ...
    '''
    merged_train = train.merge(books_, on='isbn', how='left') \
                    .merge(users_, on='user_id', how='left')
    train_df = merged_train[
        sparse_cols +
        ['summary_length', 'review_count',      # 추가된 feature
        'user_summary_merge_vector', 'book_summary_vector',
        'rating']
    ]

    merged_test = test.merge(books_, on='isbn', how='left') \
                  .merge(users_, on='user_id', how='left')
    ...
    ###################### 수정 ######################
    # 원래코드: all_df = pd.concat([train, test], axis=0)
