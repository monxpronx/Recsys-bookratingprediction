...
def process_text_data(ratings, users, books, tokenizer, model, vector_create=False):
    ...
    books_['category_clean'] = books_['category'].apply(clean_category)

    # 2) BOOK publisher 정제
    books_['publisher_clean'] = books_['publisher'].fillna('unknown').astype(str)

    # 3) USER location_country 추출
    ...
    users_['location_country'] = users_['location'].apply(extract_country)
    #####################################################################
