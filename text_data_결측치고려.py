...
def process_text_data(ratings, users, books, tokenizer, model, vector_create=False):
    ...
    users_ = users.copy()
    books_ = books.copy()
    books_['summary'] = books_['summary'].fillna("").apply(text_preprocessing)
    books_['summary'] = books_['summary'].replace({" ": ""})
    ...
    if vector_create:
        ...
        for books_read in tqdm(users_['books_read']):
            if not isinstance(books_read, list) or len(books_read) == 0:
